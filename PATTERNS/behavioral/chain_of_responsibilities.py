...
Паттерн позволяет организовать последовательную обработку запроса по цепочке проверяя может ли он обработать
или передать следующему.
1. Существую более одного объекта для обработки запроса, но заранее неизвестно,
какие запросы будут приходить и какие объекты их будут обрабатывать
2. Необходимо отправить запрос одному из ряда объектов, при этом явно не указывая какому именно
3. Обработчики запросов должны задаваться динамически
4. Обработка запросов должна производиться в строго определенной последовательности
