...
Достоинства:
1. Сокрытие деталей реализации объекта от клиентского кода
2. Разработка платформо-независимых программ
3. Возможность динамически менять реализацию в процессе работы программы

Недостатки:
1. За счет добавляемых классов может усложниться сам код программы
2.
...
class ElectricOvenImplementor(IOvenImplementor):

    def __init__(self, temperature: int = 0):
        self.temperature = temperature
        self.type = 'ElectricStove'
...
class Oven:
    def __init__(self, impelementor: IOvenImplementor):
        self.__implementor = impelementor
    ...
    def cook_pizza(self, pizza: Pizza) -> None:
        ...
        print(f"Cooking {pizza.name} pizza for {pizza.cook_time} minutes at {pizza.cook_temperature} C")
...
if __name__ == '__main__':
    ...
    nem_implementor = ElectricOvenImplementor(oven.get_temperature())
    first_pizza = Pizza('Margarita', 9, 225)
    second_pizza = Pizza('Salami', 10, 175)
    oven.change_implementor(nem_implementor)
    print(f'Implementor type: {oven.get_implementor_name()}')
    oven.cook_pizza(first_pizza)
    oven.cook_pizza(second_pizza)