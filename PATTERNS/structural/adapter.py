...
Для преобразования интерфейса одного класса в интерфейс другого класса

В каких слухаях?
1. Интерфейс существующего класса не соответствует потребностям
2. Необходимо использовать уже существующие производные классы, в которых отсутствует некоторая общая функциональность, которую
нельзя реализовать путем расширения их базового класса
"""
...
class ICelsiusOven(ABC):
    """
    Интерфейс плиты с которым будем осуществлять работу в рамках разрабатываемой системы,
...
class OvenAdapter(ICelsiusOven):
    """
    Адаптер, позволяющий работать с плитой, где единица измерения
    температуры - фаренгейты в градусах цельсия
    """
    ...
    def __init__(self, original_stove: IOven):
        ...
        self.temperature = self._int_temperature()

    def get_original_temperature(self) -> float:
        return self.stove.get_temperature()

    def _int_temperature(self) -> float:
...
if __name__ == '__main__':
    def print_temperature(stove: ICelsiusOven):
        ...
        print(f'Celsius temperature = {stove.get_celsius_temperature()} C')
